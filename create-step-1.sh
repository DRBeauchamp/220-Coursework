...
VPC_ID=$(aws ec2 create-vpc --cidr-block 10.0.0.0/16 --output json | egrep "VpcId" | cut -f2 -d : | tr -d \", | cut -d ' ' -f2)
...
IGW_ID=$(aws ec2 create-internet-gateway | egrep "igw-" | cut -f2 -d : | tr -d \", | cut -d ' ' -f2)
...
SUBNET_ID=$(aws ec2 create-subnet --cidr-block 10.0.1.0/24 --vpc-id $VPC_ID | egrep "SubnetId" | cut -f2 -d : | tr -d \", | cut -d ' ' -f2)
...
RT_ID=$(aws ec2 create-route-table --vpc-id $VPC_ID | egrep "rtb-" | cut -f2 -d : | tr -d \", | cut -d ' ' -f2)
...
KEYPAIR_ID=$(aws ec2 describe-key-pairs  --key-names Step1KeyPair | egrep "KeyPairId" | cut -f2 -d : | tr -d \", | cut -d ' ' -f2)
